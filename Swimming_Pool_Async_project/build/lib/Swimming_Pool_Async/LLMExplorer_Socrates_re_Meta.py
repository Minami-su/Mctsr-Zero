...
class LLMExplorer_Socrates:
        ...
        if self.use_meta_prompt == True:
            ...
            if self.iter > 0:
                ...
                if self.domain == "心理":
                    ...
                    template = self.prompter.Self_Meta_Refine_Prompt.format(prompt=self.Meta_Prompt,question=self.query,response=node,Critique=Critique_C)
                ...
                self.Next_Meta_Prompt = prompt.replace("##强指令##", "")
                ...
                if self.domain == "心理":
                    ...
                    method = random.choice(list(self.methods.values()))
                    template = self.prompter.Self_Meta_Refine_Prompt_init.format(method=method,system=self.Meta_Prompt)
                    pattern = r'#+\s*重写的提示词\s*#+(.*)'
                data_template1["model"] = self.api_llm.api_model
                data_template1["messages"] = [{"role": "user", "content": template}]
                prompt,_ = await self.process_controller.Generate_Response(self.api_llm, data_template1, pattern = pattern)
                self.Next_Meta_Prompt = prompt.replace("##强指令##", "").replace("##重写的提示词##", "")
                print(f"元进化提示词：{self.Next_Meta_Prompt}")
        #self.llm.tokenizer.encode()
        if self.domain == "心理":
            ...
            if self.use_meta_prompt == True:
                ...
                data_template2["messages"] = [#{"role": "system", "content": self.Meta_Prompt},
                                              {"role": "user", "content": self.query}]
            Example_Response, _ = await self.process_controller.Generate_Response(self.llm, data_template2)
    ...
    async def get_enhance_answer(self, weak_answer):
        ...
        if self.domain == "心理":
            ...
            #prompt = self.prompter.Self_Revision_Refine_Prompt.format(multi_dialog=weak_answer)
            #prompt = self.prompter.Psy_Dialogue_Enhancement_Prompt.format(multi_dialog=weak_answer,Reward_reasoning=Critique)
            if self.use_meta_prompt == True:
                data_template2["messages"] = [{"role": "system", "content": self.Meta_Prompt},
                                        {"role": "user", "content": prompt}]
            else:
            ...
            #query = self.prompter.Self_Refine_evol_Prompt.format(Question=self.query, Example_Response=weak_answer)
            query = self.prompter.Self_Critique_Enhance_Prompt.format(question=self.query,answer=weak_answer,Critique=Critique_C)
            if self.use_meta_prompt == True:
                data_template2["messages"] = [{"role": "system", "content": self.Meta_Prompt},
                                          {"role": "user", "content": query}]
            else:
                data_template2["messages"] = [{"role": "user", "content": query}]
            
            Example_Response, thinking = await self.process_controller.Generate_EnhanceResponse(self.llm, data_template2, pattern = None)#r'#+\s*回答\s*#+(.*)'
            result = Example_Response
            print("输入：",query,"\n","输出：",result)
        if "<|_error_|>" not in result:
            print("增强答案获取成功！")
    ...
    async def cal_reward(self, ans):
        ...
        if self.domain == "心理":
            ...
            prompt = self.prompter.Self_Critique_Judge_Prompt.format(query=self.query, response=ans)
            data_template3["messages"] = [{"role": "user", "content": prompt}]
            score,refine,judge = await self.process_controller.Judge_Quantity(self.llm, data_template3)
        return score*10, refine, judge
    ...
    async def sampling_reward(self, answer):
        ...
        ##########放下面是因为如果是query，那么就没有to_explore_reward，所以在计算每个节点的平均奖励时不能计算query的奖励
            ...
            reward,refine,judge = await self.cal_reward(answer)
            self.to_explore_reward[answer].append(reward)
            self.evaluations_bank[answer].append({"reward": reward, "refine": refine, "judge": judge})
        return self.to_explore_reward
    ...
    async def main_loop(self, inputs, use_weak=True):
        ...
        if self.domain == "心理": # 这部分逻辑可以保留，如果Meta_Prompt的初始值依赖它
            ...
            self.Meta_Prompt = self.system # Or a general initial Meta_Prompt
            self.query = query
